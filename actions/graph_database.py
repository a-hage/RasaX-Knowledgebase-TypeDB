...
class GraphDatabase(KnowledgeBase):
    """
    GraphDatabase uses a TypeDB graph database to encode your domain knowledege. Make
    sure to have the graph database set up and the TyprDB server running.
    """

    def __init__(self, uri: Text = "http://10.210.1.151:1729", keyspace: Text = "banking"):
        self.uri = uri
        ...
        with TypeDB.core_client(self.uri) as client:
            with client.session(self.keyspace, SessionType.DATA) as session:
                with session.transaction(TransactionType.READ) as read_transaction:
                    logger.debug("Executing Typeql Query: " + query)
                    result_iter = read_transaction.query(query)
    ...
    def _get_attribute_clause(self, attributes: Optional[List[Dict[Text, Text]]] = None) -> Text:
        ...
        clause = ""
        if attributes:
            clause = ",".join([f"has {k} '{v}'" for k,v in attributes])
            clause = ", " + clause
...
attributes= {"balance":["267.69"],'account-type':['credit']}
for k,v in attributes:
    print(k)
    print(v)
#attributes= [{'money':'mony'}]
#attributes =["category","amount"]

entities = graph.get_entities(entity_type, attributes)
